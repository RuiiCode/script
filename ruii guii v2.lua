--[[
    ...
    Author: Ruii
    Github: https://github.com/RuiiCode/script
--]]
...
export type Elements = {
	...
	AddTextBox: (self,Config: TextBox) -> { -- Nouvelle fonction
        	Option: Elements	
    	},
}
...
Ruii.Ascii = "qwertyuiopasdfghjklzxcvbnmQWRTYUIOPASDFGHJKLZXCVBNM";
Ruii.GLOBAL_ENVIRONMENT = {};
Ruii.Windows = {};
Ruii.FontSemiBold = Font.new('rbxasset://fonts/families/GothamSSm.json',Enum.FontWeight.SemiBold,Enum.FontStyle.Normal);
Ruii.Flags = {};
Ruii.Colors = {
...
function Ruii:IsMobile() : boolean
	return UserInputService.TouchEnabled;	
end;

function Ruii:RandomString() : string
...
function Ruii:GetTextSize(Text : TextLabel,CustomFont: Enum.Font) : Vector2
...
function Ruii:IsMouseOverFrame(Frame : Frame) : boolean
...
function Ruii:GetCalculatePosition(planePos: number, planeNormal: Vector3, rayOrigin: number, rayDirection: Vector3) : number
...
function Ruii:CreateHover(Element : Frame,Callback : (boolean) -> any)
...
function Ruii:GetIcon(name : string) : string
	return Ruii.Lucide['lucide-'..tostring(name)] or Ruii.Lucide[name] or Ruii.Lucide[tostring(name)] or "";
end;

function Ruii:SetIcon(name : string, value : string)
	Ruii.Lucide[name] = value;
end;

function Ruii:Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function Ruii:CreateAnimation(Instance: Instance , Time: number , Style : Enum.EasingStyle , Property : {[string] : any}) : Tween
...
function Ruii:NewInput(Frame : Frame , Callback : () -> ()) : TextButton
...
function Ruii:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local function updateInput(input)
		...
		Ruii:CreateAnimation(MoveFrame,Speed,nil,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #Ruii.DragBlacklist <= 0 then 
			dragToggle = true
	...
	UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch and #Ruii.DragBlacklist <= 0 then
			if dragToggle then
				updateInput(input)
			end
		else
			if #Ruii.DragBlacklist > 0 then
				dragToggle = false
			end
		end
	end);
...
function Ruii:ScrollSignal(Scroll: ScrollingFrame,UIListLayout: UIListLayout,Type:string)
	if Type == 'X' then
...
function Ruii:CreateResponse(args: {[string] : (any) -> any})
	local main = {};
...
function Ruii:GetWindowFromElement(Element: GuiObject)
	for i,v in next , Ruii.Windows do
...
function Ruii:CreateOption(OptionButton: ImageButton): Elements
	Ruii:CreateHover(OptionButton,function(bool)
		if bool then
			Ruii:CreateAnimation(OptionButton,0.5,{
	...
	local OwnWindow = Ruii:GetWindowFromElement(OptionButton);
	...
	Ruii:ScrollSignal(ScrollingFrame,UIListLayout,'Y');

	ExtElementFrame.Active = true;
	ExtElementFrame.Name = Ruii:RandomString()
	...
	DropShadow.Name = Ruii:RandomString()
	...
	SpaceBox.Name = Ruii:RandomString()
	...
	local ToggleExt = function(bool)
		if bool then
			...
			Ruii:CreateAnimation(ExtElementFrame,0.45,{
				Size = UDim2.new(0, 200, 0, size)
			})

			Ruii:CreateAnimation(DropShadow,0.45,{
				ImageTransparency = 0.750
			})

			Ruii:CreateAnimation(UIStroke,0.45,{
				Transparency = 0
			})
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait(0.1)
					...
					Ruii:CreateAnimation(ExtElementFrame,0.35,{
						Position = ud
					});
				end;
			end)
	...
	UserInputService.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
			if not Ruii:IsMouseOverFrame(ExtElementFrame) and not Ruii.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER then
				ToggleExt(false);
			end;
		end;
	end);

	local UIElements: Elements = Ruii:CreateElements(ScrollingFrame,ScrollingFrame.ZIndex,Bindable);

	return UIElements;
end;

function Ruii:CreateColorPicker(ColorBox: Frame,Transparency, Callback)
	Transparency = Transparency or 0;
	Callback = Callback or function() end;

	local OwnWindow = Ruii:GetWindowFromElement(ColorBox);
	...
	local VisibleToggle = function(value)
		if value then
			...
			Ruii:CreateAnimation(ColorPickBox,0.45,{
				ImageTransparency = 0,
				BackgroundTransparency = 0
			});

			Ruii:CreateAnimation(MouseMovement,0.45,{
				ImageTransparency = 0,
			});

			Ruii:CreateAnimation(UIStroke_2,0.45,{
				Transparency = 0
			});

			Ruii:CreateAnimation(ColorRedGreenBlue,0.45,{
				BackgroundTransparency = 0
			});

			Ruii:CreateAnimation(ColorRGBSlide,0.45,{
				BackgroundTransparency = 0
			});

			Ruii:CreateAnimation(UIStroke_3,0.45,{
				Transparency = 0.75
			});

			Ruii:CreateAnimation(ColorOpc,0.45,{
				BackgroundTransparency = 0
			});

			Ruii:CreateAnimation(ColorOptSlide,0.45,{
				BackgroundTransparency = 0
			});

			Ruii:CreateAnimation(UIStroke_4,0.45,{
				Transparency = 0.75
			});

			Ruii:CreateAnimation(UIStroke_5,0.45,{
				Transparency = 0
			});

			Ruii:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.45
			});

			Ruii:CreateAnimation(CopyButton,0.45,{
				ImageTransparency = 0.45
			});

			Ruii:CreateAnimation(hexCode,0.45,{
				BackgroundTransparency = 0.4
			});

			Ruii:CreateAnimation(HexCodeText,0.45,{
	...
	ColorPickerFrame.Name = Ruii:RandomString()
	...
	Ruii:AddDragBlacklist(ColorPickerFrame);
	...
	ColorPickBox.Name = Ruii:RandomString()
	...
	MouseMovement.Name = Ruii:RandomString()
	...
	ColorRedGreenBlue.Name = Ruii:RandomString()
	...
	ColorRGBSlide.Name = Ruii:RandomString()
	...
	ColorOpc.Name = Ruii:RandomString()
	...
	ColorOptSlide.Name = Ruii:RandomString()
	...
	ColorOpt.Name = Ruii:RandomString()
	...
	PasteButton.Name = Ruii:RandomString()
	...
	CopyButton.Name = Ruii:RandomString()
	...
	hexCode.Name = Ruii:RandomString()
	hexCode.Parent = ColorOpt
	hexCode.BackgroundColor3 = Ruii.Colors.Black
	...
	HexCodeText.Name = Ruii:RandomString()
	...
	HexCodeText.FontFace = Ruii.FontSemiBold
	...
	Ruii:CreateHover(CopyButton,function(bool)
		if bool then
			Ruii:CreateAnimation(CopyButton,0.45,{
	...
	Ruii:CreateHover(PasteButton,function(bool)
		if bool then
			Ruii:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.1
			})
	...
	PasteButton.MouseButton1Click:Connect(function()
		if Ruii.GLOBAL_ENVIRONMENT.COLOR_COPY then
			local h,s,v = Ruii.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHSV();

			Transparency = Ruii.GLOBAL_ENVIRONMENT.COLOR_COPY.OPC;

			OldCode = h;
			CodeH = s;
			CodeV = v;

			HexCodeText.Text = "#" .. tostring(Ruii.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHex());
			...
			ColorOpc.BackgroundColor3 = Ruii.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB;
	...
	CopyButton.MouseButton1Click:Connect(function()
		Ruii.GLOBAL_ENVIRONMENT.COLOR_COPY = {
	...
	Ruii:NewInput(ColorBox,function()
		VisibleToggle(true);
	end);
	...
	do
		...
		ColorPickerFrame.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				Ruii.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = true;
		...
		ColorPickerFrame.InputEnded:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				IsPressM1 = false;
				Ruii.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = false;
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if not Ruii:IsMouseOverFrame(ColorPickerFrame) then
					VisibleToggle(false);
				end;
			end;
		end)
		...
		ColorRedGreenBlue.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait()
					...
					Ruii:CreateAnimation(ColorBox,0.5,{
						BackgroundColor3 = Color
					});

					Ruii:CreateAnimation(ColorOpc,0.35,{
						BackgroundColor3 = Color
					});

					Ruii:CreateAnimation(ColorPickBox,0.5,{
		...
		ColorPickBox.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait();
					...
					Ruii:CreateAnimation(MouseMovement,0.2,nil,{
						Position = UDim2.new(CodeH, 0, 1 - CodeV, 0)
					})
				end
			end
...
function Ruii:RandomStr(len: number): string
	local main = "";

	for i = 1 , len do
		local max = #Ruii.Ascii;
		local rand = math.random(1,max);

		main = main .. Ruii.Ascii:sub(rand,rand);
	end;

	return main;
end;

function Ruii:AddDragBlacklist(Frame: Frame)
	Frame.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			local finder = table.find(Ruii.DragBlacklist , Frame);

			if not finder then
				table.insert(Ruii.DragBlacklist , Frame);
			end;
		end;
	end);
	...
	Frame.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if finder then
				table.remove(Ruii.DragBlacklist , finder);
			end;
		end;
	end);
end;

function Ruii:ProtectText(Label: TextLabel,Text: string)
	Label.RichText = true;

	local MainText = string.gsub(Text,'.',function(t)
		if not string.find(t,"[<>()\"']") then
			return string.format('<font %s="%s">%s</font>',Ruii:RandomStr(5),Ruii:RandomStr(5),t)
		end;
		return t;
	end);
...
function Ruii:CreateDropdown(Parent: Frame, Default: string | {[string]: boolean}, Multiplier: boolean, AutoUpdate: boolean,Callback: (data: any) -> any)
	local Window = Ruii:GetWindowFromElement(Parent);
	local Data = {};
	...
	Ruii:AddDragBlacklist(DropdownItemFrame);
		...
		if value then
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait()
					...
					Ruii:CreateAnimation(DropdownItemFrame,0.35,{
						Size = UDim2.new(0, 175, 0, math.clamp(baseSize,0,200))
					})
				end;
			end)

			Ruii:CreateAnimation(UIStroke,0.35,{
				Transparency = 0
			})

			Ruii:CreateAnimation(DropShadow,0.35,{
				ImageTransparency = 0.75
			})
		else
			if SPAWN_THREAD then
	...
	DropdownItemFrame.Name = Ruii:RandomString()
	...
	local new_button = function()
		...
		db_selected.Name = Ruii:RandomString()
		...
		db_selected.FontFace = Ruii.FontSemiBold
		db_selected.TextColor3 = Ruii.Colors.Main
			...
			for i,v in next , Data do
				...
				if Multiplier then
					if (typeof(Default) == 'table' and (Default[v] or table.find(Default,v))) or Default == v then
						Selected[v] = true
						bth.TextColor3 = Ruii.Colors.Main;
					else
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Ruii:IsMouseOverFrame(DropdownItemFrame) then
				func(false);
				Toggle(false);
			end;
		end;
...
function Ruii:CreateElements(Parent : Frame , ZIndex : number , Event : BindableEvent,SearchAPI: {Path: string,Memory : (name:string) -> any}) : Elements
	local elements = {};
	local FatalWindow = Ruii:GetWindowFromElement(Parent);
	...
	function elements:AddToggle(Config: Toggle)
		...
		Ruii:AddDragBlacklist(Toggle);

		Toggle_Name.Name = Ruii:RandomString()
		...
		Toggle_Name.FontFace = Ruii.FontSemiBold;
		...
		Ruii:ProtectText(Toggle_Name,Config.Name);

		ValueFrame.Name = Ruii:RandomString()
		ValueFrame.Parent = Toggle
		ValueFrame.AnchorPoint = Vector2.new(1, 0.5)
		ValueFrame.BackgroundColor3 = Ruii.Colors.Black
		...
		ValueIcon.Name = Ruii:RandomString()
		...
		OptionButton.Name = Ruii:RandomString()
		...
		local toggleImg = function(value)
			if value then
				Ruii:CreateAnimation(ValueIcon,0.45,{
					ImageTransparency = 0,
					ImageColor3 = Ruii.Colors.Main,
		...
		local OpcToggle = function(value)
			if value then
				...
				Ruii:CreateAnimation(OptionButton,0.45,{
					ImageTransparency = (Config.Option and 0.600) or 1
				})

				toggleImg(Config.Default)

				Ruii:CreateAnimation(ValueFrame,0.45,{
					BackgroundTransparency = 0,
				})

				Ruii:CreateAnimation(Toggle_Name,0.45,{
		...
		Ruii:CreateHover(ValueFrame,function(b)
		...
		Ruii:NewInput(ValueFrame,function()
			Config.Default = not Config.Default;
			toggleImg(Config.Default);
			Config.Callback(Config.Default)
		end);

		local Respons = Ruii:CreateResponse({
			Rename = function(new_name)
				Toggle_Name.Text = new_name;
				Ruii:ProtectText(Toggle_Name,new_name);
			end,
			GetValue = function()
			...
			Option = (Config.Option and Ruii:CreateOption(OptionButton)) or nil;
		});

		if Config.Flag then
			Ruii.WindowFlags[FatalWindow][Config.Flag.."Toggle"] = Respons;
		end;
	...
	function elements:AddSlider(Config: Slider)
		...
		Ruii:AddDragBlacklist(Slider);

		Slider_Name.Name = Ruii:RandomString()
		...
		Slider_Name.FontFace = Ruii.FontSemiBold
		...
		boxli.Name = Ruii:RandomString()
		boxli.Parent = ValueFrame
		boxli.BackgroundColor3 = Ruii.Colors.Main
		...
		ValueText.Name = Ruii:RandomString()
		...
		ValueText.FontFace = Ruii.FontSemiBold
		...
		local OpcToggle = function(value)
			if value then
				Ruii:CreateAnimation(boxli,0.45,{
					BackgroundTransparency = 0,
				})
				...
				Ruii:CreateAnimation(Slider_Name,0.45,{
					TextTransparency = 0.200
				})

				Ruii:CreateAnimation(ValueText,0.45,{
		...
		local function update(Input)
			...
			local Value = Ruii:Rounding(Main,Config.Round);
			...
			ValueFrame.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not Ruii:IsMouseOverFrame(ValueFrame) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		if Config.Flag then
			Ruii.WindowFlags[FatalWindow][Config.Flag.."Slider"] = Respons;
		end;
	...
	function elements:AddButton(Config: Button)
		...
		Button.Name = Ruii:RandomString()
		Button.Parent = Parent
		Button.BackgroundColor3 = Ruii.Colors.Black
		Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Button.BorderSizePixel = 0
		Button.Size = UDim2.new(1, -25, 0, 25)
		Button.ZIndex = ZIndex + 1
		Ruii:AddDragBlacklist(Button);

		Button_Name.Name = Ruii:RandomString()
		...
		Button_Name.FontFace = Ruii.FontSemiBold
		...
		Ruii:ProtectText(Button_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				Ruii:CreateAnimation(Button_Name,0.45,{
					TextTransparency = 0.4,
				})

				Ruii:CreateAnimation(Button,0.45,{
					BackgroundTransparency = 0,
				})
			else
				Ruii:CreateAnimation(Button_Name,0.45,{
		...
		Ruii:CreateHover(Button,function(value)
			if value then
				Ruii:CreateAnimation(Button,0.45,{
				...
				Ruii:CreateAnimation(Button,0.45,{
					BackgroundColor3 = Ruii.Colors.Black
				})
			end;
		end)

		Ruii:NewInput(Button,function()
			Config.Callback();
		end)

		return Ruii:CreateResponse({
			Rename = function(new_name)
				Button_Name.Text = new_name
				Ruii:ProtectText(Button_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
	...
	function elements:AddColorPicker(Config: ColorPicker)
		...
		ColorPicker.Name = Ruii:RandomString()
		...
		Ruii:AddDragBlacklist(ColorPicker);

		ColorPicker_Name.Name = Ruii:RandomString()
		...
		ColorPicker_Name.FontFace = Ruii.FontSemiBold
		...
		local res = Ruii:CreateColorPicker(ValueFrame,Config.Transparency,function(rgb,opc)
		...
		local OpcToggle = function(value)
			if value then
				Ruii:CreateAnimation(ColorPicker_Name,0.45,{
		...
		if Config.Flag then
			Ruii.WindowFlags[FatalWindow][Config.Flag.."ColorPicker"] = Respons;
		end;
	...
	function elements:AddDropdown(Config: Dropdown)
		...
		Dropdown.Name = Ruii:RandomString()
		...
		Ruii:AddDragBlacklist(Dropdown);

		Dropdown_Name.Name = Ruii:RandomString()
		...
		Dropdown_Name.FontFace = Ruii.FontSemiBold
		...
		Ruii:ProtectText(Dropdown_Name,Config.Name);
		...
		Value_Text.Name = Ruii:RandomString()
		...
		Value_Text.FontFace = Ruii.FontSemiBold
		...
		local res;
		res = Ruii:CreateDropdown(ValueFrame,Config.Default,Config.Multi,Config.AutoUpdate,function(args)
			Config.Default = args;
			Value_Text.Text = DataParser(Config.Default);
		...
		res:on_toggle(function(b)
			if b then
				Ruii:CreateAnimation(icon,0.35,{
					Rotation = -180,
					ImageColor3 = Ruii.Colors.Main
				})
			else
		...
		local OpcToggle = function(value)
			if value then
				Ruii:CreateAnimation(Dropdown_Name,0.45,{
					TextTransparency = 0.2,
				})
				...
				Ruii:CreateAnimation(icon,0.45,{
					ImageTransparency = 0
				})

				Ruii:CreateAnimation(Value_Text,0.45,{
		...
		local Respons = Ruii:CreateResponse({
			Rename = function(new_name)
				...
				Ruii:ProtectText(Dropdown_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			Ruii.WindowFlags[FatalWindow][Config.Flag.."Dropdown"] = Respons;
		end;
	...
	function elements:AddKeybind(Config: Keybind)
		...
		Keybind.Name = Ruii:RandomString()
		...
		Ruii:AddDragBlacklist(Keybind);

		Keybind_Name.Name = Ruii:RandomString()
		...
		Keybind_Name.FontFace = Ruii.FontSemiBold
		...
		Ruii:ProtectText(Keybind_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				Ruii:CreateAnimation(Keybind_Name,0.45,{
					TextTransparency = 0.2,
				})
		...
		local Respons = Ruii:CreateResponse({
			Rename = function(new_name)
				...
				Ruii:ProtectText(Keybind_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			...
			Ruii.WindowFlags[FatalWindow][Config.Flag.."Keybind"] = Respons;
		end;
	...
	function elements:AddTextBox(Config: TextBox)
    		...
    		TextBox.Name = Ruii:RandomString()
    		TextBox.Parent = Parent
    		TextBox.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    		TextBox.BackgroundTransparency = 1.000
    		TextBox.BorderColor3 = Color3.fromRGB(0, 0, 0)
    		TextBox.BorderSizePixel = 0
    		TextBox.Size = UDim2.new(1, -25, 0, 17)
    		TextBox.ZIndex = ZIndex + 1
    		Ruii:AddDragBlacklist(TextBox);

    		TextBox_Name.Name = Ruii:RandomString()
    		TextBox_Name.Parent = TextBox
    		...
    		TextBox_Name.FontFace = Ruii.FontSemiBold;
    		TextBox_Name.Text = Config.Name
    		TextBox_Name.TextColor3 = (Config.Risky and Color3.fromRGB(255, 160, 92)) or Color3.fromRGB(255, 255, 255)
    		TextBox_Name.TextSize = 13.000
    		TextBox_Name.TextTransparency = 1
    		TextBox_Name.TextXAlignment = Enum.TextXAlignment.Left;

    		Ruii:ProtectText(TextBox_Name,Config.Name);

    		ValueFrame.Name = Ruii:RandomString()
    		ValueFrame.Parent = TextBox
    		ValueFrame.AnchorPoint = Vector2.new(1, 0.5)
    		ValueFrame.BackgroundColor3 = Ruii.Colors.Black
    		ValueFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
    		ValueFrame.BorderSizePixel = 0
    		...
    		TextInput.Name = Ruii:RandomString()
    		TextInput.Parent = ValueFrame
    		...
    		TextInput.Position = UDim2.new(0, 3, 0, 0)
    		TextInput.Size = UDim2.new(1, -6, 1, 0)
    		TextInput.ZIndex = ZIndex + 3
    		TextInput.FontFace = Ruii.FontSemiBold
    		TextInput.PlaceholderText = Config.Placeholder
    		TextInput.Text = Config.Default
    		TextInput.TextColor3 = Color3.fromRGB(255, 255, 255)
    		TextInput.TextSize = 10.000
    		...
    		local OpcToggle = function(value)
        		if value then
            		Ruii:CreateAnimation(TextBox_Name,0.45,{
                		TextTransparency = 0.200
            		})
            		...
            		Ruii:CreateAnimation(TextInput,0.45,{
    		...
    		-- Gestion des événements du TextBox
    		...
    		Ruii:CreateHover(ValueFrame,function(bool)
        		if bool then
            		Ruii:CreateAnimation(ValueFrame,0.25,{
    		...
    		local Respons = Ruii:CreateResponse({
        		Rename = function(new_name)
            		...
            		Ruii:ProtectText(TextBox_Name,new_name);
        		end,
        		GetValue = function()
        		...
        		SetValue = function(value)
            		local IsSame = TextInput.Text == value;
    		...
    		if Config.Flag then
        		Ruii.WindowFlags[FatalWindow][Config.Flag.."TextBox"] = Respons;
    		end;
...
function Ruii:CreateConfigWindow(Root: ScreenGui , Fatal , Button: ImageButton)
	...
	Ruii:AddDragBlacklist(ConfigWindowFrame);
	...
	local ElementToggle = function(value)
		...
		if value then
			...
			Ruii:CreateAnimation(ConfigWindowFrame,0.3,{
				Size = UDim2.new(0, 185, 0, 195),
			})

			Ruii:CreateAnimation(UIStroke,0.3,{
				Transparency = 0
			})

			Ruii:CreateAnimation(DropShadow,0.3,{
				ImageTransparency = 0.750
			})

			Ruii:CreateAnimation(ScrollingFrame,0.3,{
				BackgroundTransparency = 0
			})

			Ruii:CreateAnimation(UIStroke_2,0.3,{
				Transparency = 0.750
			})

			Ruii:CreateAnimation(UIStroke_3,0.3,{
				Transparency = 0.750
			})

			Ruii:CreateAnimation(SpaceBox,0.3,{
				BackgroundTransparency = 0
			})

			Ruii:CreateAnimation(InputFrame,0.3,{
				BackgroundTransparency = 0
			})

			Ruii:CreateAnimation(TextBox,0.3,{
				TextTransparency = 0.5
			})

			Ruii:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			Ruii:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			Ruii:CreateAnimation(SaveConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			Ruii:CreateAnimation(DeleteButton,0.3,{
			...
			Ruii:CreateAnimation(ConfigWindowFrame,0.275,{
	...
	ScrollingFrame.Active = true
	ScrollingFrame.BackgroundColor3 = Ruii.Colors.Black
	ScrollingFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
	ScrollingFrame.BorderSizePixel = 0
	...
	InputFrame.BackgroundColor3 = Ruii.Colors.Black
	...
	TextBox.ZIndex = 209
	TextBox.FontFace = Ruii.FontSemiBold
	TextBox.Text = ""
	...
	DeleteButton.ImageColor3 = Ruii.Colors.Main
	...
	Ruii:CreateHover(DeleteButton,function(bool)
		if bool then
			Ruii:CreateAnimation(DeleteButton,0.3,{
				ImageTransparency = 0;
			})
	...
	Ruii:CreateHover(SaveConfigButton,function(bool)
		if bool then
			Ruii:CreateAnimation(SaveConfigButton,0.3,{
	...
	Ruii:CreateHover(GetConfigButton,function(bool)
		if bool then
			Ruii:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	Ruii:CreateHover(AddConfigButton,function(bool)
		if bool then
			Ruii:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	game:GetService('UserInputService').InputBegan:Connect(function(Input,Typing)
		if Input.UserInputType == Enum.UserInputType.Touch or Input.UserInputType == Enum.UserInputType.MouseButton1 then
			if UIToggle and not Ruii:IsMouseOverFrame(ConfigWindowFrame) then
				ElementToggle(false);
			end;
		end;
	end);
	...
	res = Ruii:CreateResponse({
		...
		Refresh = function()
			...
			for i,v in next, Configs do
				...
				if tostring(import.Name) == tostring(v) or import.Name == v then
					...
					import.Name = v;
					import.Element.TextColor3 = Ruii.Colors.Main;
					button.Text = "  "..tostring(v);
				end;
				...
				button.MouseButton1Click:Connect(function()
					...
					import.Element = button;
					import.Name = v;
					button.TextColor3 = Ruii.Colors.Main;
					button.Text = "  "..tostring(v);
				end);
			end;
		end,
		...
		Init = function(Name: string,Folder: string)
			if not isfolder(Folder or "Ruii") then
				makefolder(Folder or "Ruii");	
			end;

			if not isfolder((Folder or "Ruii").."/Config") then
				makefolder((Folder or "Ruii").."/Config");	
			end;

			local cfgPath = (Folder or "Ruii").."/Config/"..tostring(Name);
...
function Ruii.new(Window: Window)
	Window = Window or {};
	Window.Name = Window.Name or "Ruii";
	...
	Fatal.Notifier = Ruii.__NOTIFIER_CACHE or Ruii:CreateNotifier();

	local Ruiiwin = Instance.new("ScreenGui")
	...
	Ruii.WindowFlags[Ruiiwin] = {};

	Ruii:ScrollSignal(tbc,UIListLayout,'X');
	...
	FatalFrame:GetPropertyChangedSignal("BackgroundTransparency"):Connect(function()
		if FatalFrame.BackgroundTransparency >= 0.99 then
			Ruiiwin.Enabled = false;
		else
			Ruiiwin.Enabled = true;
		end;
	end);
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			Ruii:CreateAnimation(FatalFrame,0.15,{
				Size = Window.Scale,
				BackgroundTransparency = 0,
			})

			Ruii:CreateAnimation(Header,0.5,{
				BackgroundTransparency = 0,
			})

			Ruii:CreateAnimation(HeaderLine,0.5,{
				BackgroundTransparency = 0,
			})

			Ruii:CreateAnimation(Bottom,0.5,{
				BackgroundTransparency = 0,
			})

			Ruii:CreateAnimation(HeaderLine_2,0.5,{
				BackgroundTransparency = 0,
			})

			Ruii:CreateAnimation(HeaderLineShadow,0.5,{
				BackgroundTransparency = 0.5,
			})

			Ruii:CreateAnimation(HeaderLineShadow_2,0.5,{
				BackgroundTransparency = 0.5,
			})

			Ruii:CreateAnimation(InfoButton,0.25,{
				ImageTransparency = 0.5
			})

			Ruii:CreateAnimation(SearchButton,0.25,{
				ImageTransparency = 0.5
			})

			Ruii:CreateAnimation(SaveButton,0.25,{
				ImageTransparency = 0.5
			})

			Ruii:CreateAnimation(HeaderText,0.35,{
				TextStrokeTransparency = 0.640,
				TextTransparency = 0
			})

			Ruii:CreateAnimation(UserIcon,0.45,{
				ImageTransparency = 0,
				Position = UDim2.new(1, -10,0.5, 0)
			})

			Ruii:CreateAnimation(User_name,0.35,{
				TextTransparency = 0,
				Position = UDim2.new(1, -40,0, 3)
			})

			Ruii:CreateAnimation(expire_days,0.5,{
				TextTransparency = 0,
			...
			Ruii:CreateAnimation(InfoButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(1, -5,0.5, 0)
			})

			Ruii:CreateAnimation(SearchButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(0,10,0.5, 0)
			})

			Ruii:CreateAnimation(SaveButton,0.1,Enum.EasingStyle.Back,{
			...
			table.clear(Ruii.DragBlacklist);

			for i,v in next , Fatal.Menus do
				v.ValueSelect(false);
			end;

			Ruii:CreateAnimation(SaveButton,0.35,{
				Position = UDim2.new(0,35,1, 20)
			})

			Ruii:CreateAnimation(SearchButton,0.35,{
				Position = UDim2.new(0,10,1, 20)
			})

			Ruii:CreateAnimation(InfoButton,0.35,{
			...
			Ruii:CreateAnimation(UserIcon,0.35,{
				ImageTransparency = 1,
				Position = UDim2.new(1, -10,0.5, 0)
			})
			...
			Ruii:CreateAnimation(expire_days,0.1,{
				TextTransparency = 1,
				Position = UDim2.new(1, -40,0, 16)
			})
			...
			Ruii:CreateAnimation(FatalFrame,0.75,{
				BackgroundTransparency = 1,
			})
	...
	Ruiiwin.Name = Ruii:RandomString();
	Ruiiwin.Parent = CoreGui;
	Ruiiwin.ResetOnSpawn = false;
	Ruiiwin.IgnoreGuiInset = true;
	Ruiiwin.ZIndexBehavior = Enum.ZIndexBehavior.Global;

	table.insert(Ruii.Windows,Ruiiwin)

	protect_gui(Ruiiwin);

	FatalFrame.Active = true;
	FatalFrame.Name = Ruii:RandomString()
	FatalFrame.Parent = Ruiiwin
	...
	HeaderLine.Name = Ruii:RandomString()
	...
	HeaderText.Name = Ruii:RandomString()
	...
	MenuButtonCont.Name = Ruii:RandomString()
	...
	UserProfle.Name = Ruii:RandomString()
	...
	UserIcon.Name = Ruii:RandomString()
	...
	User_name.Name = Ruii:RandomString()
	...
	expire_days.Name = Ruii:RandomString()
	...
	HeaderLineShadow.Name = Ruii:RandomString()
	...
	MenuFrame.Name = Ruii:RandomString()
	...
	HeaderLine_2.Name = Ruii:RandomString()
	...
	HeaderLineShadow_2.Name = Ruii:RandomString()
	...
	Ruii:Drag(FatalFrame,FatalFrame,0.1);
	...
	function Fatal:GetFlags()
		return Ruii.WindowFlags[Ruiiwin];
	end;
	...
	function Fatal:AddConfig()
		...
		Ruii:CreateHover(ConfigButton,function(bool)
			if bool then
				Ruii:CreateAnimation(ConfigButton,0.3,{
					ImageTransparency = 0.2500;
				})
			else
				Ruii:CreateAnimation(ConfigButton,0.3,{
		...
		return Ruii:CreateConfigWindow(Ruiiwin,Fatal,ConfigButton);
	end;
	...
	function Fatal:LoadConfig(config)
		...
		for i,v in next , config do
			if i ~= "Info" then
				local Element = Ruii.WindowFlags[Ruiiwin][i];
	...
	function Fatal:AddMenu(Menu : Menu)
		...
		MenuButton.Name = Ruii:RandomString()
		...
		Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
		Icon.ZIndex = 5
		Icon.Image = Ruii:GetIcon(Menu.Icon);
		Icon.ImageColor3 = Ruii.Colors.Main
		Icon.ScaleType = Enum.ScaleType.Crop

		UICorner_2.CornerRadius = UDim.new(1, 0)
		UICorner_2.Parent = Icon

		menu_name.Name = Ruii:RandomString()
		...
		local text_size = Ruii:GetTextSize(menu_name);
		...
		MenuLiber.Name = Ruii:RandomString()
		...
		BindEvent.Name = Ruii:RandomString();

		if not Menu.AutoFill then
			Ruii:ScrollSignal(Right,UIListLayout_3,'Y');
			Ruii:ScrollSignal(Center,UIListLayout_2,'Y');
			Ruii:ScrollSignal(Left,UIListLayout,'Y');
		else
		...
		Ruii:CreateHover(MenuButton,function(bool)
		...
		Ruii:NewInput(MenuButton,function()
			for i,v in next , Fatal.Menus do
		...
		function MenuLib:AddPreview(Config: Preview)
			...
			PreviewName.FontFace = Ruii.FontSemiBold
			...
			local Toggle = function(v)
				if v then
					Ruii:CreateAnimation(Preview,0.5,{
						BackgroundTransparency = 0
					})

					Ruii:CreateAnimation(PreviewName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
					
					Ruii:CreateAnimation(Main,0.5,{
						BackgroundTransparency = 0
					})
					...
					Ruii:CreateAnimation(MainBlock,0.5,{
		...
		function MenuLib:AddListBox(Config : Listbox)
			...
			ListBox.Name = Ruii:RandomString()
			...
			Ruii:AddDragBlacklist(ScrollingFrame);

			Elements.Name = Ruii:RandomString()
			...
			SearchBox.Name = Ruii:RandomString()
			SearchBox.Parent = Elements
			SearchBox.BackgroundColor3 = Ruii.Colors.Black
			...
			TextBox.BackgroundColor3 = Ruii.Colors.Black
			TextBox.BackgroundTransparency = 1.000
			TextBox.BorderColor3 = Color3.fromRGB(0, 0, 0)
			TextBox.BorderSizePixel = 0
			...
			ListName.Name = Ruii:RandomString()
			...
			ListName.ZIndex = 10
			ListName.FontFace = Ruii.FontSemiBold
			...
			local Toggle = function(v)
				if v then
					Ruii:CreateAnimation(ListBox,0.5,{
						BackgroundTransparency = 0
					})
					...
					Ruii:CreateAnimation(UIStroke_2,0.5,{
						Transparency = 0.75
					})

					Ruii:CreateAnimation(UIStroke_3,0.5,{
						Transparency = 0.75
					})

					Ruii:CreateAnimation(SearchBox,0.5,{
						BackgroundTransparency = 0
					})

					Ruii:CreateAnimation(TextBox,0.5,{
						TextTransparency = 0
					})
					...
					Ruii:CreateAnimation(ScrollingFrame,0.45,{
						BackgroundTransparency = 0,
					...
					table.foreach(ScrollingFrame:GetChildren(),function(i,v)
						if v:IsA('TextButton') then
							Ruii:CreateAnimation(v,0.45,{
								TextTransparency = 0.5
							});
						end;
					end);

					Ruii:CreateAnimation(ListName,0.5,{
				...
				Refresh = function()
					...
					for i,v in next , Config.Values do
						...
						if Config.Multi then
							...
							Ruii:CreateHover(bth,function(bool)
								if not Config.Default[v] then
									if bool then
										Ruii:CreateAnimation(bth,0.25,{
											TextTransparency = 0;
										})
									else
										Ruii:CreateAnimation(bth,0.25,{
			...
			if Config.Flag then
				Ruii.WindowFlags[Ruiiwin][Config.Flag.."ListBox"] = res;
			end;
		...
		function MenuLib:AddSection(Config : Section)
			...
			local Toggle = function(v)
				if v then
					Ruii:CreateAnimation(Section,0.5,{
						BackgroundTransparency = 0
					})
					...
					Ruii:CreateAnimation(SectionName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
				else
			...
			Section.Name = Ruii:RandomString()
			...
			SectionName.Name = Ruii:RandomString()
			...
			SectionName.FontFace = Ruii.FontSemiBold;
			...
			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				...
				if not Menu.AutoFill then
					Ruii:CreateAnimation(Section,0.25,{
			...
			return Ruii:CreateElements(Elements,Elements.ZIndex,BindEvent,{
				Path = Menu.Name .. " > ".. Config.Name,
	...
	do
		InfoButton.Name = Ruii:RandomString()
		...
		SearchButton.Name = Ruii:RandomString()
		...
		SaveButton.Name = Ruii:RandomString()
		...
		Ruii:CreateHover(SaveButton,function(bool)
			if bool then
				Ruii:CreateAnimation(SaveButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				Ruii:CreateAnimation(SaveButton,0.5,{
		...
		Ruii:CreateHover(InfoButton,function(bool)
			if bool then
				Ruii:CreateAnimation(InfoButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				Ruii:CreateAnimation(InfoButton,0.5,{
	...
	do
		...
		Ruii:AddDragBlacklist(SearchFrame);
		...
		local SearchToggle = function(value)
			if value then
				...
				Ruii:CreateAnimation(SearchFrame,0.35,{
					Size = searchScale
				})
				...
				Ruii:CreateAnimation(SearchBox,0.35,{
					BackgroundTransparency = 0
				})
		...
		SearchFrame.Name = Ruii:RandomString()
		SearchFrame.Parent = Ruiiwin;
		...
		Ruii:CreateHover(SearchButton,function(bool)
			if bool then
				Ruii:CreateAnimation(SearchButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				Ruii:CreateAnimation(SearchButton,0.5,{
		...
		local get_button = function(Name,Path,TAB_WARP)
			...
			ResultFrame.Name = Ruii:RandomString()
			ResultFrame.Parent = ScrollingFrame;
			ResultFrame.BackgroundColor3 = Ruii.Colors.Black
			...
			FeatureName.Name = Ruii:RandomString()
			...
			FeatureName.FontFace = Ruii.FontSemiBold
			...
			FeaturePath.Name = Ruii:RandomString()
			...
			FeaturePath.FontFace = Ruii.FontSemiBold
			...
			local button = Ruii:NewInput(ResultFrame);

			Ruii:CreateHover(button,function(bool)
				if bool then
					Ruii:CreateAnimation(ResultFrame,0.5,{
						BackgroundTransparency = 0
					})
				else
					Ruii:CreateAnimation(ResultFrame,0.5,{
		...
		SearchButton.MouseButton1Click:Connect(function()
			...
			table.foreach(Fatal.ElementContents,function(i,v)
				...
				SearchInformation[v.Path.." - "..Ruii:RandomString()] = {
					root = button,
					callback = function()
		...
		UserInputService.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				if not Ruii:IsMouseOverFrame(SearchFrame) then
					SearchToggle(false);
				end
			end
		end)
	...
	function Fatal:GetButton()
		...
		backpack.Name = "Ruii"..Ruii:RandomString();
		...
		backpack.MouseEnter:Connect(function()
			Ruii:CreateAnimation(backpack,0.3,nil,{
				BackgroundColor3 = Ruii.Colors.Main,
...
function Ruii:Loader(Config: Loader)
	Config = Config or {};
	Config.Name = Config.Name or "Ruii";
	...
	BlackFrame.Name = Ruii:RandomString()
	...
	Ruii:CreateAnimation(Blur,1,{
		Size = 60
	})

	Ruii:CreateAnimation(BlackFrame,0.5,{
	...
	local createText = function(TEXT)
		...
		local textsize = Ruii:GetTextSize(ASCII);
	...
	do
		...
		StartText.Name = Ruii:RandomString()
		...
		local textsize = Ruii:GetTextSize(StartText);
		...
		Ruii:CreateAnimation(StartText,0.45,{
			TextTransparency = 0
		})

		Ruii:CreateAnimation(UIScale,0.5,{
			Scale = Config.Scale;
		});

		task.wait(0.45);

		Ruii:CreateAnimation(StartText,0.35,{
		...
		for i,v in next , PosText do
			if i > 1 then
				Ruii:CreateAnimation(v.Text,0.65,{
					Position = UDim2.new(0.5,0,0.5,0);
					TextTransparency = 0
				})
			end;
		...
		Ruii:CreateAnimation(StartText,1.5,{
			TextTransparency = 1
		})

		for i,v in next , PosText do
			Ruii:CreateAnimation(v.Text,1.5,{
				TextTransparency = 1
			})
		end;

		Ruii:CreateAnimation(Blur,1.5,{
			Size = 0
		})

		Ruii:CreateAnimation(BlackFrame,1.5,{
			BackgroundTransparency = 1
		})
...
function Ruii:CreateNotifier(): Notifier
	if Ruii.__NOTIFIER_CACHE then return Ruii.__NOTIFIER_CACHE; end;

	local Notify = Instance.new("ScreenGui")
	local layout = Instance.new("Frame")
	local UIListLayout = Instance.new("UIListLayout")

	Notify.Name = Ruii:RandomString();
	Notify.Parent = CoreGui
	Notify.ResetOnSpawn = false
	Notify.ZIndexBehavior = Enum.ZIndexBehavior.Global
	Notify.IgnoreGuiInset = true;

	layout.Name = Ruii:RandomString();
	...
	local res = Ruii:CreateResponse({
		Notify = function(Config: Notify)
			...
			notify_block.Name = Ruii:RandomString()
			...
			Icon.BackgroundColor3 = Ruii.Colors.Main
			Icon.BackgroundTransparency = 1.000
			Icon.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Icon.BorderSizePixel = 0
			Icon.Position = UDim2.new(0, 7, 0, 7)
			Icon.Size = UDim2.new(0, 18, 0, 18)
			Icon.Image = Ruii:GetIcon(Config.Icon);
			Icon.ImageColor3 = Ruii.Colors.Main
			...
			HeaderText.FontFace = Ruii.FontSemiBold
			HeaderText.Text = Config.Title
			HeaderText.TextColor3 = Ruii.Colors.Main
			HeaderText.TextSize = 13.000
			HeaderText.TextTransparency = 1
			HeaderText.TextXAlignment = Enum.TextXAlignment.Left

			BodyText.Name = Ruii:RandomString()
			...
			BodyText.ZIndex = 55
			BodyText.FontFace = Ruii.FontSemiBold
			...
			local updateScale = function()
				local TitleScale = Ruii:GetTextSize(HeaderText,Enum.Font.GothamBold);
				local ContentScale = Ruii:GetTextSize(BodyText,Enum.Font.GothamMedium);
				...
				Ruii:CreateAnimation(notify_block,0.35,{
					Position = UDim2.new(0.5, 0, 0, 0)
				});
			end;

			task.delay(0.1,function()
				Ruii:CreateAnimation(notify_block,0.25,{
					BackgroundTransparency = 0.1
				})

				Ruii:CreateAnimation(UIStroke,0.25,{
					Thickness = 2.500,
					Transparency = 0.900
				})

				Ruii:CreateAnimation(Icon,0.25,{
					ImageTransparency = 0
				})

				Ruii:CreateAnimation(HeaderText,0.25,{
					TextTransparency = 0.200
				})

				Ruii:CreateAnimation(BodyText,0.25,{
			...
			task.delay(Config.Duration + 0.25,function()
				...
				task.delay(0.35,function()
					Ruii:CreateAnimation(notify,0.5,{
						Size = UDim2.new(0,0,0,0)
					})
	...
	Ruii.__NOTIFIER_CACHE = res;

	return res;
end;

Ruii.Ruii_PID = Ruii:RandomString();
